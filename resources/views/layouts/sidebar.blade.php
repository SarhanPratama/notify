<ul class="navbar-nav sidebar sidebar accordion" id="accordionSidebar">
    ...
    @canany(['kas.view', 'pembelian.view', 'penjualan.view'])
        <li class="nav-item">
            <a class="nav-link collapsed fw-bold" href="#" data-toggle="collapse" data-target="#collapseKas"
                aria-expanded="true" aria-controls="collapseKas">
                <i class="fa fa-money text-maron"></i>
                <span>Transaksi</span>
            </a>
            <div id="collapseKas"
                class="collapse {{ in_array(request()->route()->getName(), ['transaksi.index', 'transaksi.create', 'piutang.index', 'pembelian.index', 'pembelian.create', 'penjualan.index', 'penjualan.create', 'penjualan.edit']) ? 'show' : '' }}"
                ...
                <div class="bg-white py-2 collapse-inner rounded">
                    ...
                    @can('pembelian.view')
                        <a class="collapse-item {{ request()->routeIs(['pembelian.index', 'pembelian.create', 'pembelian.edit']) ? 'active font-weight-bold' : '' }}"
                            href="{{ route('pembelian.index') }}">Pembelian</a>
                    @endcan
                    @can('penjualan.view')
                        <a class="collapse-item {{ request()->routeIs(['penjualan.index', 'penjualan.create', 'penjualan.edit']) ? 'active font-weight-bold' : '' }}"
    ...
    @canany(['supplier.view', 'bahan-baku.view'])
        <li class="nav-item">
            <a class="nav-link collapsed fw-bold" href="#" data-toggle="collapse" data-target="#collapseInventory"
                aria-expanded="true" aria-controls="collapseInventory">
                <i class="fa fa-truck text-maron"></i>
                <span>Inventory</span>
            </a>
            <div id="collapseInventory"
                class="collapse {{ in_array(request()->route()->getName(), ['supplier.index', 'bahan-baku.index']) ? 'show' : '' }}"
                aria-labelledby="headingInventory" data-parent="#accordionSidebar">
                <div class="bg-white py-2 collapse-inner rounded">
                    @can('supplier.view')
                        <a class="collapse-item {{ request()->routeIs('supplier.index') ? 'active font-weight-bold' : '' }}"
    ...
    @can('laporan.view')
        <li class="nav-item">
            ...
            <div id="collapselaporan"
                class="collapse {{ in_array(request()->route()->getName(), ['laporan-stok', 'laporan-pembelian', 'laporan-penjualan']) ? 'show' : '' }}"
                ...
                <div class="bg-white py-2 collapse-inner rounded">
                    <a class="collapse-item {{ request()->routeIs('laporan-stok') ? 'active font-weight-bold' : '' }}"
                        href="{{ route('laporan-stok') }}">Stok Bahan Baku</a>
                    <a class="collapse-item {{ request()->routeIs('laporan-pembelian') ? 'active font-weight-bold' : '' }}"
                        href="{{ route('laporan-pembelian') }}">Pembelian</a>
                    <a class="collapse-item {{ request()->routeIs('laporan-penjualan') ? 'active font-weight-bold' : '' }}"
                        href="{{ route('laporan-penjualan') }}">Penjualan</a>
                </div>
            </div>
        </li>
    @endcan
    ...
    @canany(['users.view', 'cabang.view', 'role.view', 'permission.view', 'akses-role.manage'])
        ...
        <li class="nav-item">
            ...
            <div id="collapseUser"
                class="collapse {{ in_array(request()->route()->getName(), ['users.index', 'users.create', 'users.show', 'users.edit', 'cabang.index', 'role.index', 'permission.index', 'akses-role.index']) ? 'show' : '' }}"
