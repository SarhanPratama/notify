            ...
            <div class="col-lg-12">
                ...
                <div class="card shadow-sm border-0 mb-4 rounded-lg overflow-hidden">
                    ...
                    <div class="card-body bg-white p-4">
                        {{-- Pastikan route dan variabel dari controller ($showTable, $tanggalMulai, $tanggalSampai, $laporan_penjualan) sudah benar --}}
                        <form method="GET" action="{{ route('laporan-penjualan') }}" class="row g-3 align-items-end">
                            ...
                            <div class="col-md-4">
                                <div class="d-flex gap-2">
                                    ...
                                    @if(isset($showTable) && $showTable && isset($laporan_penjualan) && $laporan_penjualan->isNotEmpty())
                                        <a href="{{ route('laporan-penjualan.pdf', ['tanggal_mulai' => $tanggalMulai, 'tanggal_sampai' => $tanggalSampai]) }}"
                                            target="_blank"
                ...
                {{-- Conditional display of data table --}}
                    ...
                    <div class="card shadow-sm border-0">
                        <div
                            ...
                            <h6 class="font-weight-bold">
                                {{-- Menampilkan judul Laporan Penjualan dengan rentang tanggal yang difilter --}}
                                @if(isset($tanggalMulai) && isset($tanggalSampai))
                                    Laporan Penjualan ({{ \Carbon\Carbon::parse($tanggalMulai)->format('d M Y') }} - {{ \Carbon\Carbon::parse($tanggalSampai)->format('d M Y') }})
                                @else
                                    {{ $breadcrumbs[count($breadcrumbs) - 1]['label'] ?? 'Laporan Penjualan' }}
                                @endif
                            </h6>
                        </div>
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover table-striped text-sm mb-0" id="dataTableHover">
                                    ...
                                    <tbody>
                                        @forelse ($laporan_penjualan as $item)
                                            <tr class="border-bottom">
                                                ...
                                                <td class="align-middle font-weight-bold">{{ $item->bahanBaku->nama ?? 'N/A' }}</td>
                                                <td class="align-middle font-weight-bold">
                                            ...
                                            <tr>
                                                <td colspan="8" class="text-center p-4">
                                                    <div class="alert alert-info mb-0">
                                                        ...
                                                        Tidak ada data penjualan yang ditemukan untuk rentang tanggal yang dipilih.
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                    @if(isset($laporan_penjualan) && $laporan_penjualan->isNotEmpty())
                                    <tfoot>
                                        <tr>
                                            <th colspan="7" style="text-align: right;" class="fw-bold">Total Pemasukkan</th>
                                            <th class="text-danger fs-5 fw-bold">
                ...
                @else
                    {{-- Pesan jika filter belum diterapkan --}}
                    <div class="card shadow-sm border-0 mb-4 rounded-lg overflow-hidden">
                        <div class="card-body bg-white p-5 text-center">
                            <i class="fas fa-filter fa-3x text-primary mb-3"></i>
                            <h5 class="font-weight-bold text-primary mb-2">Laporan Penjualan</h5>
                            <p class="text-secondary">Silakan pilih rentang tanggal dan klik "Filter Data" untuk menampilkan laporan penjualan.</p>
                        </div>
                    </div>
                @endif
            </div>
